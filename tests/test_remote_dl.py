...
class TestRemoteDL:
    ...
    def test_dl_uncompress(self):
        ...
        with gzip.GzipFile(self.RESOURCES_PATH.joinpath(filename)) as gz_handle:
    ...
    def test_1(self, status):
        ...
        try:
            with pytest.raises(requests.exceptions.RetryError) as e:
                requests_retry(retries=retries, backoff=backoff).get(
                    f"http://httpbin.org/status/{status}"
                )
        finally:
