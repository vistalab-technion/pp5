...
def main():
    ...
    usage = {aa: g.cod.value_counts(normalize=True).to_dict()
             for aa, g in df.groupby("AA")}
    ...
    for (aa, ss), g in df.groupby(["AA", "condition_group"]):
        ...
        means = {c: (cmean(g[g.codon == c].phi), cmean(g[g.codon == c].psi)) for c in cods}
        for i in range(len(cods)):
            for j in range(i + 1, len(cods)):
                ca, cb = cods[i], cods[j]
                fa = usage[aa][ca.split("-")[1]]; fb = usage[aa][cb.split("-")[1]]
    ...
    print(f"     Spearman(|Δusage|, torus-dist-between-means) rho={rho:.3f}  p={pv:.2e}")
    print(f"     usage_gap quartiles -> mean ang_dist:")
    P["qbin"] = pd.qcut(P.usage_gap, 4, labels=["Q1", "Q2", "Q3", "Q4"])
    print(P.groupby("qbin", observed=True).ang_dist.agg(["mean", "median", "count"]).round(2).to_string())

    # ---- #1b: directionality for the robust pairs -----------------------------
    print("\n#1b  robust pairs — usage, optimal codon, oriented shift (optimal−rare):")
    print(f"{'pair':<16}{'SS':<6}{'f(c1)':>6}{'f(c2)':>6}  {'optimal':<8}"
          f"{'Δφ':>6}{'Δψ':>6}{'dist':>6}")
    db = []
    for ss, c1, c2 in PAIRS:
        f1 = usage[c1.split('-')[0]][c1.split('-')[1]]
        f2 = usage[c2.split('-')[0]][c2.split('-')[1]]
        m1, _ = codon_mean(df, ss, c1); m2, _ = codon_mean(df, ss, c2)
        opt, rare = (c1, c2) if f1 >= f2 else (c2, c1)
        mo = m1 if opt == c1 else m2; mr = m1 if rare == c1 else m2
        dphi = ((mo[0] - mr[0] + 180) % 360) - 180
        dpsi = ((mo[1] - mr[1] + 180) % 360) - 180
        print(f"{c1+':'+c2.split('-')[1]:<16}{ss:<6}{f1:>6.2f}{f2:>6.2f}  {opt:<8}"
              f"{dphi:>6.1f}{dpsi:>6.1f}{tdist(mo, mr):>6.1f}")
        db.append(dict(pair=f"{c1}:{c2}", SS=ss, f_c1=round(f1, 3), f_c2=round(f2, 3),
                       optimal=opt, dphi=round(dphi, 1), dpsi=round(dpsi, 1),
                       dist=round(tdist(mo, mr), 1)))
    pd.DataFrame(db).to_csv("out/pnas-2026-repro/codon_optimality_pairs.csv", index=False)

    # ---- #2: expression stratification ---------------------------------------
    gene_opt = df.assign(f=[usage[a][c] for a, c in zip(df.AA, df.cod)]) \
                 .groupby("unp_id").f.mean()
    df["gene_opt"] = df.unp_id.map(gene_opt)
    qs = df.gene_opt.quantile([1 / 3, 2 / 3]).values
    df["expr_bin"] = np.where(df.gene_opt <= qs[0], "LOW",
                              np.where(df.gene_opt <= qs[1], "MID", "HIGH"))
    ...
    for ss, c1, c2 in PAIRS:
        for b in ["LOW", "MID", "HIGH"]:
            ...
            print(f"{c1+':'+c2.split('-')[1]:<16}{ss:<6}{b:<6}{len(A):>5}{len(B):>5}"
                  f"{dist:>7.1f}{p:>9.4f}")
            s2.append(dict(pair=f"{c1}:{c2}", SS=ss, expr_bin=b, n1=len(A), n2=len(B),
                           dist=round(dist, 1) if dist == dist else None,
                           perm_p=round(p, 4) if p == p else None))
    pd.DataFrame(s2).to_csv("out/pnas-2026-repro/codon_optimality_strat.csv", index=False)
