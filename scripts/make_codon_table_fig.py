...
V = {
 ...
 "UAU":2.8,"UAC":2.3,"UAA":25,"UAG":27,    "UGU":4.4,"UGC":2.0,"UGA":24,"UGG":2.4,
...
cmap = mcolors.LinearSegmentedColormap.from_list("pk", ["white", "#f7c5da", "#ec5c9f", "#c2185b"])
norm = mcolors.Normalize(0.7, 9.2)

fig, ax = plt.subplots(figsize=(12, 8.5))
ax.set_xlim(-0.55, 4.35); ax.set_ylim(-0.4, 4.6); ax.axis("off"); ax.set_aspect("equal")
ax.text(1.9, 4.5, "Second position of codon", ha="center", fontsize=13)
ax.text(-0.5, 2, "First position of codon (5′ end)", rotation=90, va="center", fontsize=13)
ax.text(4.3, 2, "Third position of codon (3′ end)", rotation=90, va="center", fontsize=13)
...
for i, b1 in enumerate(B):
    ...
    for j, b2 in enumerate(B):
        ...
        k = 0
        while k < 4:
            aa = AA[b1 + b2 + B[k]]; k2 = k
            while k2 + 1 < 4 and AA[b1 + b2 + B[k2 + 1]] == aa:
                k2 += 1
            ymid = (cys[k] + cys[k2]) / 2
            ax.text(x0 + 0.30, ymid, aa, ha="center", va="center",
                    fontsize=12 if len(aa) <= 3 else 9, color="#39a0db", weight="bold")
            k = k2 + 1
        for k, b3 in enumerate(B):
            cod = b1 + b2 + b3; v = V[cod]; cy = cys[k]
            stop = AA[cod] == "Stop"
            fc = "#9c0026" if stop else cmap(norm(v))
            ax.add_patch(Ellipse((x0 + 0.72, cy), 0.30, 0.18, fc=fc, ec="k", lw=0.8))
            ax.text(x0 + 0.72, cy, f"{v:g}", ha="center", va="center", fontsize=9.5,
                    weight="bold", color="white" if (stop or v >= 5) else "black")
            ax.text(x0 + 0.95, cy, b3, ha="center", va="center", fontsize=8.5, color="0.45")
plt.tight_layout()
