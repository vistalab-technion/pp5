...
def main():
    ...
    print(f"# positions w/ abundance {len(d)}/{len(df)} ({100*len(d)/len(df):.0f}%)\n")
    print(f"{'pair':<14}{'SS':<6}{'bin':<6}{'n1':>5}{'n2':>5}{'MMD2u p':>9}{'sig':>5}{'MMD2u':>9}")
    rows = []
    for ss, c1, c2 in PAIRS:
        ...
        for b in ["ALL", "LOW", "MID", "HIGH"]:
            ...
            if len(A) < MINN or len(B) < MINN:
                print(f"{c1+':'+c2.split('-')[1]+tag:<14}{ss:<6}{b:<6}{len(A):>5}{len(B):>5}{'(small)':>9}")
                continue
            K = kmat(np.vstack([A, B])); nx, ny = len(A), len(B)
            m = mmd2(K[:nx, :nx].sum(), K[nx:, nx:].sum(), K[:nx, nx:].sum(), nx, ny, "u")
            _, p = perm_p(K, nx, ny, "u")
            sig = p <= THR[ss]
            print(f"{c1+':'+c2.split('-')[1]+tag:<14}{ss:<6}{b:<6}{nx:>5}{ny:>5}{p:>9.4f}"
                  f"{('yes' if sig else 'no'):>5}{m:>9.4f}")
            rows.append(dict(pair=f"{c1}:{c2}", SS=ss, robust=f"{c1}:{c2}" in ROBUST, bin=b,
                             n1=nx, n2=ny, mmd2u_p=round(p, 5), sig=sig, mmd2u=round(m, 4)))
        print()
    pd.DataFrame(rows).to_csv("out/pnas-2026-repro/expression_confound_mmd.csv", index=False)
    print("saved: out/pnas-2026-repro/expression_confound_mmd.csv   (* = robust-across-stats pair)")
