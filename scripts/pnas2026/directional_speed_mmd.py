...
def spd(c):
    return SPD.get(c.split("-")[1].replace("T", "U"))
...
def main():
    ...
    for (aa, ss), g in df.groupby(["AA", "condition_group"]):
        ...
        for c in cods:
            ...
            if len(v) > CAP:
                v = v[rng.choice(len(v), CAP, replace=False)]
            ang[c] = v
        for i in range(len(cods)):
            for j in range(i + 1, len(cods)):
                ...
                rows.append(
                    (aa, ss, ca, cb, min(len(ang[ca]), len(ang[cb])), lsr, m, rob)
                )
    P = pd.DataFrame(
        rows, columns=["AA", "SS", "cA", "cB", "minn", "lsr", "mmd2u", "robust"]
    )
    P.to_csv("out/pnas-2026-repro/directional_speed_mmd.csv", index=False)
    print(f"pairs: {len(P)}  (codons capped at {CAP})")
    print(
        "Spearman(min n, MMD2u)            rho=%+.3f p=%.2g  (confirms size-unbiased)"
        % spearmanr(P.minn, P.mmd2u)
    )
    print(
        "Spearman(|log speed-ratio|, MMD2u) rho=%+.3f p=%.2g"
        % spearmanr(P.lsr, P.mmd2u)
    )
    print("\nrobust pairs:")
    print(
        P[P.robust][["SS", "cA", "cB", "minn", "lsr", "mmd2u"]]
        .round(4)
