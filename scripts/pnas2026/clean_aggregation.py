...
def cmean_cols(df, by):
    ...
    gg = df.assign(
        cphi=np.cos(r_phi), sphi=np.sin(r_phi), cpsi=np.cos(r_psi), spsi=np.sin(r_psi)
    ).groupby(by)
    m = gg[["cphi", "sphi", "cpsi", "spsi"]].transform("mean")
    return (
        np.rad2deg(np.arctan2(m["sphi"], m["cphi"])),
...
def main():
    ...
    dp = pd.read_csv(
        DP,
        usecols=["pdb_id", "unp_id", "unp_idx", "secondary", "phi", "psi"],
        low_memory=False,
    ...
    centre = (
        inl.assign(cphi=c2["cphi"], cpsi=c2["cpsi"])
        .groupby(BY)[["cphi", "cpsi"]]
        .first()
    )
    ...
    outlier = (d2 > T) & (dp["n"] >= 3)
    n_out = (
        pd.Series(outlier.values, index=dp.index)
        .groupby([dp[b] for b in BY])
        .transform("sum")
    )
    ...
    agg = (
        clean.assign(phi_c=cphi, psi_c=cpsi)
        .groupby(BY)
        .agg(
            phi_clean=("phi_c", "first"),
            psi_clean=("psi_c", "first"),
            n_clean=("phi", "size"),
        )
        .reset_index()
    )
    ...
    rr = clean.assign(
        cp=np.cos(np.deg2rad(clean.phi)),
        spp=np.sin(np.deg2rad(clean.phi)),
        cs=np.cos(np.deg2rad(clean.psi)),
        ss=np.sin(np.deg2rad(clean.psi)),
    ).groupby(BY)
    Rphi = np.hypot(rr.cp.mean(), rr.spp.mean()).clip(1e-12, 1)
    Rpsi = np.hypot(rr.cs.mean(), rr.ss.mean()).clip(1e-12, 1)
    std = pd.DataFrame(
        {
            "std_clean": np.maximum(
                np.rad2deg(np.sqrt(-2 * np.log(Rphi))),
                np.rad2deg(np.sqrt(-2 * np.log(Rpsi))),
            )
        }
    ).reset_index()
    ...
    out = ds.merge(
        agg, left_on=["unp_id", "unp_idx", "condition_group"], right_on=BY, how="left"
    )
    ...
    npos_changed = (
        int(
            (
                ...
                .size()
                .reset_index()["phi"]
                .reindex(agg.set_index(BY).index)
                .values
            ).sum()
        )
        if False
        else len(removed.groupby(["unp_id", "unp_idx", "SS"]))
    )
    print(f"T = {T} deg")
    print(
        f"structures removed: {len(removed)} "
        f"({100*len(removed)/len(dp):.3f}% of all per-structure rows)"
    )
    print(
        f"positions affected: {removed.groupby(['unp_id','unp_idx','SS']).ngroups} "
