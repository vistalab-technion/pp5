...
def main():
    dp = pd.read_csv(DP, usecols=["pdb_id", "unp_id", "res_id", "unp_idx", "phi", "psi"],
                     low_memory=False)
    ...
    if REMOVED:
        ...
        removed_set = set(zip(rm["unp_id"], rm["unp_idx"].astype(int),
                              rm["pdb"].astype(str) + ":" + rm["chain"].astype(str)))
        if removed_set:
            mask = [(u, i, p) in removed_set for u, i, p in
                    zip(dp["unp_id"], dp["unp_idx"], dp["pdb_id"].astype(str))]
    ...
    for arm, path in ARMS.items():
        ...
        for _, row in w.iterrows():
            ...
            for _, s in sub.iterrows():
                ...
                detail.append(dict(
                    arm=arm, SS=row["SS"], pair=row["pair"], rank=int(row["rank"]),
                    codon=row["codon"], unp_id=row["unp_id"], unp_idx=int(row["unp_idx"]),
                    pdb_id=str(pid).split(":")[0], chain=chain, res_id=s["res_id"],
                    phi=round(float(s["phi"]), 1), psi=round(float(s["psi"]), 1),
                    # aggregate stats repeated per row (full angles + the summary together)
                    n_pdb=len(sub), phi_mean=round(phbar, 1), phi_cstd=round(phsd, 1),
                    psi_mean=round(psbar, 1), psi_cstd=round(pssd, 1), wide_spread=wide))
            summary.append(dict(
                arm=arm, SS=row["SS"], pair=row["pair"], rank=int(row["rank"]),
                codon=row["codon"], unp_id=row["unp_id"], unp_idx=int(row["unp_idx"]),
                n_pdb=len(sub),
                phi_mean=round(phbar, 1), phi_cstd=round(phsd, 1),
                psi_mean=round(psbar, 1), psi_cstd=round(pssd, 1), wide_spread=wide))
    ...
    md = [f"## Appendix {LABEL} — kill-set samples: contributing structures and "
    ...
    for arm in ARMS:
        ...
        for (ss, pair), grp in sa.groupby(["SS", "pair"], sort=False):
            ...
            for _, r in grp.iterrows():
                pos = f"{r['unp_id']}:{r['unp_idx']}" + (" **WIDE**" if r["wide_spread"] else "")
                if r["n_pdb"] > 1:  # only report spread when there is more than one structure
                    phi = f"{r['phi_mean']:+.1f} ± {r['phi_cstd']:.1f}"
                    psi = f"{r['psi_mean']:+.1f} ± {r['psi_cstd']:.1f}"
                else:
                    phi = f"{r['phi_mean']:+.1f}"
                    psi = f"{r['psi_mean']:+.1f}"
                di = dd[(dd.arm == arm) & (dd.pair == pair) &
                        (dd.unp_id == r["unp_id"]) & (dd.unp_idx == r["unp_idx"])]
                pdbs = "; ".join(f"{x.pdb_id}:{x.chain},{x.res_id}" for x in di.itertuples())
                md.append(f"| {r['rank']} | {r['codon']} | {pos} | {phi} | {psi} | "
                          f"{r['n_pdb']} | {pdbs} |")
    ...
    print(f"{len(wide)} of {len(ds)} kill-set entries flagged WIDE "
          f"({(ds.n_pdb > 1).sum()} are multi-structure):")
    if len(wide):
        print(wide[["arm", "SS", "pair", "codon", "unp_id", "unp_idx", "n_pdb",
                    "phi_mean", "phi_cstd", "psi_mean", "psi_cstd"]].to_string(index=False))
    m = ds[ds.n_pdb > 1]
