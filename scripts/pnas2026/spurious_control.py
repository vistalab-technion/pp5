...
def embed(ang):  # (n,2) radians -> circular 4D
    return np.column_stack(
        [np.cos(ang[:, 0]), np.sin(ang[:, 0]), np.cos(ang[:, 1]), np.sin(ang[:, 1])]
    )
...
def tune_noise(ang, n1, n2, target_p, ss, rng):
    ...
    for _ in range(9):
        ...
        _, p = perm_p(K, len(A), len(B), "u", k=2000)
...
def main():
    ...
    for ss, aa, n1, n2, tp, real_bk in REAL:
        pool = np.deg2rad(
            df[(df.condition_group == ss) & (df.AA == aa)][["phi", "psi"]].values
        )
        print(
            f"=== match {aa}-pair in {ss}: n=({n1},{n2}), target p~{tp}, "
        ...
        for r in range(NREP):
            ...
            bks.append(bk if bk is not None else max(kg) + 1)
            ps.append(p)
        bks = np.array([b if b is not None else 0 for b in bks])
        print(
            f"  spurious p: median={np.median(ps):.4f}; "
            f"spurious breakdown-k: {sorted(bks)}  median={int(np.median(bks))} max={int(bks.max())}"
        )
        print(
            f"  --> REAL breakdown-k = {real_bk}  vs spurious median {int(np.median(bks))} "
