...
def main():
    ...
    print(
        f"# positions with PaxDb abundance: {len(d)}/{len(df)} ({100*cov:.0f}%); "
    ...
    for ss, c1, c2 in PAIRS:
        for b in ["ALL", "LOW", "MID", "HIGH"]:
            ...
            if len(A) < MINN or len(B) < MINN:
                print(
                    f"{c1+':'+c2.split('-')[1]:<14}{ss:<6}{b:<6}{len(A):>5}{len(B):>5}"
                    f"{'(small)':>10}"
                )
                continue
            ...
            rows.append(
                dict(
                    ...
                    kde_l1_p=round(p, 5),
                    significant=sig,
                    sized_dist=round(sized, 3),
                )
            )
        print()
    pd.DataFrame(rows).to_csv(
        "out/pnas-2026-repro/expression_confound.csv", index=False
    )
