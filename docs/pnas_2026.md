...
full pipeline run or the original DVN download to work), this repo also carries two static
copies under `out/pnas-2026/`, which ships as a zip alongside the submission:

- **`out/pnas-2026/dataset-raw/`** — a copy of the raw dataset above (`data-precs.csv`,
  `meta.json`), byte-identical to the DVN-sourced original.
- **`out/pnas-2026/dataset-processed/dataset.csv`** — a copy of the aggregated/preprocessed
...
- The dataset folder `out/prec-collected/20211001_124553-aida-ex_EC-src_EC/` present (only
  ...
  `robustness-figures.ipynb`, which read the static copies under `out/pnas-2026/` -- see
...
`BASE_PATH` already points at `out/pnas-2026/` (the real run output, not a legacy path) and
...
The figure is saved as `out/pnas-2026/figs/pvals_cdf-FDR=0.05.pdf`.
...
R1-R4. See that notebook's own markdown cells for details; outputs land under
`out/pnas-2026/robustness/`.
...
Input data: `out/pnas-2026/dataset-processed/dataset.csv` (aggregated, post-preprocessing --
the default every script below reads) and `out/pnas-2026/dataset-raw/data-precs.csv` (raw
